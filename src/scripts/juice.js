...
const juiceTemp = document.querySelector(".card");

juices.forEach((juice) => juiceContainer.append(createProduct(juice)));

function createProduct(juice) {
  const clone = juiceTemp.content.cloneNode(true);
  ...
  image.setAttribute("alt", juice.name);
  heading.innerText = juice.name;
  description.innerText = juice.description;
  price.innerText = juice.price;
  available.innerText = juice.available + "Availble";
  rating.innerText = juice.ratings + " |";
  review.innerText = juice.reviews + " Reviews";
  stars.innerHTML = createStars(juice.ratings);

  if (juice.tag) {
    const tagElem = document.createElement("div");
    tagElem.setAttribute("class", "");
    tagElem.className = "btn-secondary absolute top-28 right-0";
    tagElem.innerText = juice.tag.toUpperCase();
